...
def _ocp_semver_expr(constraint: str) -> str:
    ...
    op = m.group(1) or ">="
    ver = m.group(2)
    # Normalize .Values.targetOCPVersion to major.minor.0 so both "4.20" and a
    # full "4.20.1" (as emitted by `oc get clusterversion`) yield valid semver.
    norm = ('(printf "%s.%s.0" '
            '(.Values.targetOCPVersion | toString | splitList "." | first) '
            '(index (.Values.targetOCPVersion | toString | splitList ".") 1))')
...
def values_yaml(contents: list[Content], layer: str, content_version: str) -> str:
    ...
    out = [
        ...
        f'targetOCPVersion: "{DEFAULT_OCP_VERSION}"',
        "",
    ...
    out += [
        ...
        "# Pre-populated below: for each group of mutually-exclusive alternatives the",
        "# losers are disabled so whitelisting a whole profile renders out of the box.",
        "# Flip these to choose a different alternative.",
    ]
    disabled = default_disabled_rules(contents)
...
{PLATFORM_CHART}:
  targetOCPVersion: "{DEFAULT_OCP_VERSION}"
  profiles: {{}}
  rules: {{}}
  tailoredProfile:
    enabled: false
...
# RULES.md matrix
# --------------------------------------------------------------------------- #
def rules_matrix(contents: dict[str, Content], version: str) -> str:
    """Build a markdown matrix: rule | target object | severity | layer |
    product | profiles, with mutually-exclusive alternatives marked."""
    content_list = list(contents.values())
    ...
    header = [
        ...
        "",
        "| Rule | Target object | Severity | Layer | Product | Profiles |",
        "|------|---------------|----------|-------|---------|----------|",
    ]
    return "\n".join(header + rows) + "\n"
